...
<html>
	...
	<body>
		<div id="body">
			...
			<!--删除商品-->
			<div id="deleteWaitPayOrder" >
				<p style="margin-left: 244px; margin-top: 16px; color: #178EE0; font-size: 18px; font-family: '微软雅黑';"  >确定删除该订单</p>
				<button  onclick="hideDeleteWaitPayOrderDialog()" class="redButton" style="position: absolute; right: 16px; bottom: 50px;">取消</button>
				<button  onclick="confirmHideDeleteWaitPayOrderDialog('${devbuy_web_path}')" class="blueButton"  style="position: absolute; right: 95px; bottom: 50px;" >确定</button>
			</div>
				<!--模板内容区域右侧开始-->
		<div id="ContentRight_2_2_1">
			<div id="ContentRightTitle_2_2_1">
				<button onclick="showDeleteWaitPayOrderDialog()" class="contentButton" id="btnDeleteClass" >删除</button>
				<input type="text" placeholder="搜索用户名/订单号"></input>
				<img src="${devbuy_web_path}/img/iv_search_query.png" />
			</div>
			<form  name="deleteWaitPayOrdersIds" >
			<table>
				<tr>
					<td><input type="checkbox"   id="ck_content_2_2_1"  />  <p id="ContentRightFirtst_2_2_1">全选</p></td>
					<td><p>订单号</p></td>
					<td><p>用户名</p></td>
					<td><p>购买商品</p></td>
					<td><p>促销价</p></td>
				...
				<c:forEach   items="${ordersCustoms }"   var="ordersCustom" >
					<tr>
						...
						<td><button  type="button"  onclick="showEditWaitPayOrderDialog('${devbuy_web_path}','${ordersCustom.ordersId}')"  >查看详情</button></td>
					</tr>
				</c:forEach>
			</table>
		</form>
	...
	<script  type="text/javascript" >
	 $("#ck_content_2_2_1").change(function(){
		 
			if($("#ck_content_2_2_1").prop("checked")){
				  $(".WaitPayOrdersCK").each(function(){
